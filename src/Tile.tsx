interface TileProps {
  ...
  const Tile: React.FC<TileProps & { pictures: string[]; setTiles: React.Dispatch<React.SetStateAction<string[]>> }> = ({ url, index, pictures, setTiles }) => {
    const refreshOne = () => {
      const newUrl = pictures[Math.floor(Math.random() * pictures.length)];
      setTiles((prevTiles: any) => {
        const updatedTiles = [...prevTiles];
        updatedTiles[index] = newUrl;
        return updatedTiles;
      });
    };
  
    return (
      <div className="image-container" onClick={refreshOne}>
        <div className="base-image" style={{ backgroundImage: `url(${url})` }}></div>
